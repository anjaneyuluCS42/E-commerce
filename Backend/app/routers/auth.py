...
async def register_user(
    ...
    for existing_user in existing_users:
        if existing_user.email == user.email:
            raise HTTPException(
                status_code=400,
                detail="Email already exists. Please login using your password.",
            )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code in (200, 201):
                    ...
                    logger.info(
                        f"Successfully registered user in Supabase Auth: {user.email}"
                    )
                elif resp.status_code == 400 and (
                    "already" in resp.text.lower() or "use another" in resp.text.lower()
                ):
                    # User exists in Supabase, but was deleted from public.users locally
                    supabase_auth_success = True
                    logger.info(
                        f"User already registered in Supabase Auth, continuing with local creation: {user.email}"
                    )
                else:
                    logger.error(
                        f"Supabase Auth signup returned: {resp.status_code} - {resp.text}"
                    )
                    try:
                        err_data = resp.json()
                        error_msg = err_data.get(
                            "msg", "Failed to register with authentication service."
                        )
    ...
    if not supabase_auth_success and not SMTP_USER:
        response_data["test_verification_url"] = (
            f"{backend_url}/auth/verify?token={verification_token}"
        )
    ...
    if db_user and supabase_anon_key:
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code in (200, 201):
                    logger.info(
                        f"Successfully sent password recovery email via Supabase to: {payload.email}"
                    )
                else:
                    logger.error(
                        f"Supabase Auth recover returned: {resp.status_code} - {resp.text}"
                    )
        except Exception as e:
...
async def reset_password(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if not email:
                raise HTTPException(
                    status_code=400, detail="Could not retrieve email from token."
                )
    except Exception as e:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    logger.info(
                        f"Successfully authenticated user with Supabase Auth: {form_data.username}"
                    )
                    ...
                    if (
                        "confirm" in err_msg.lower()
                        or "verify" in err_msg.lower()
                        or "confirmed" in err_msg.lower()
                    ):
                        raise HTTPException(
                            status_code=400,
                        ...
                        logger.warning(
                            f"Supabase login failed for {form_data.username}: {err_msg}. Trying local authentication fallback."
                        )
                else:
                    logger.error(
                        f"Supabase Auth login returned: {resp.status_code} - {resp.text}"
                    )
        except HTTPException:
            raise
        except Exception as e:
    ...
    if supabase_login_success:
        if not db_user.is_verified:
            try:
                ...
                logger.warning(
                    f"Could not sync verification status to local db: {sync_err}"
                )
    else:
...
async def refresh_token(
    ...
    new_access_token = create_access_token(
        {"sub": user.email, "role": user.role, "id": user.id, "username": user.username}
    )

...
async def check_task_ownership(task_id: str, current_user):
    ...
    if owner_id_str:
        ...
        if current_user.role != 'admin':
            raise HTTPException(status_code=403, detail="Access denied. Task owner unknown.")

@router.post("/test-hello", response_model=dict)
async def trigger_hello_task(request: TestTaskRequest, current_user = Depends(get_current_user)):

...
async def place_order(
    ...
    for pid_str, qty_str in cart_items.items():
        ...
        if not product or product.stock < qty:
            raise HTTPException(
                status_code=400, detail=f"Stock issue for product {pid}"
            )
        ...
        order_items_to_add.append(
            OrderItem(product_id=product.id, quantity=qty, price=product.price)
        )
    ...
    try:
        ...
        logging.getLogger("uvicorn").error(
            f"Failed to clear products cache on checkout: {e}"
        )

    # Fire background task and generate PDF Invoice
    serializable_items = [
        {
            "name": product_dict.get(item.product_id).name,
    ...
    task = send_order_confirmation_email.delay(
        current_user.email, order.id, total_price, serializable_items
    )
...
async def update_order_status(
    ...
    if status_input == "Delivered" and order.payment_method in [
        "Cash on Delivery",
        "COD",
    ]:
    ...
    if (
        not history
        or latest_status != status_input
        or (
            status_data.current_location
            and latest_location != status_data.current_location
        )
    ):
        history.append(
            {
...
async def trigger_invoice_generation(
    ...
    task = generate_invoice_task.delay(
        order.id, current_user.email, order.total_price, items_list
    )

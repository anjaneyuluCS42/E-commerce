...
class AIService:
    @staticmethod
    async def _call_llm(
        messages: List[Dict[str, Any]], temperature: float = 0.4
    ) -> str:
        """
        ...
        for provider in providers:
            if not provider["api_key"]:
                logger.warning(
                    f"Skipping {provider['name']} as API key is not configured."
                )
                continue

            try:
                logger.info(
                    f"Attempting to query LLM using provider: {provider['name']} (model: {provider['model']})"
                )
                async with httpx.AsyncClient() as httpx_client:
                    ...
                    response = await httpx_client.post(
                        provider["url"], headers=headers, json=payload, timeout=30.0
                    )
                    ...
                    if content:
                        logger.info(
                            f"LLM call successfully completed using provider: {provider['name']}"
                        )
                        return content
        ...
        raise last_error or RuntimeError(
            "No LLM providers available or all of them failed."
        )
    ...
    async def _call_llm_stream(
        ...
        for provider in providers:
            if not provider["api_key"]:
                logger.warning(
                    f"Skipping {provider['name']} stream as API key is not configured."
                )
                continue

            try:
                logger.info(
                    f"Attempting to stream from LLM using provider: {provider['name']} (model: {provider['model']})"
                )
                headers = {
                    "Authorization": f"Bearer {provider['api_key']}",
                    "Content-Type": "application/json",
                        ...
                        async for line in response.aiter_lines():
                            if line.startswith("data: "):
                                ...
                                try:
                                    ...
                                    pass
                logger.info(
                    f"LLM streaming successfully completed using provider: {provider['name']}"
                )
    ...
    async def generate_chat_response(message: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        has_strict_criteria = (
            criteria["category"]
            or criteria["brand"]
            or criteria["budget_min"]
            or criteria["budget_max"]
        )
        ...
        for prod in db_products:
            ...
            if criteria["category"]:
                is_mobile_query = criteria["category"] in [
                    "mobile",
                    "phone",
                    "smartphone",
                    "cellphone",
                ]
                is_laptop_query = criteria["category"] in [
                    "laptop",
                    "computer",
                    "pc",
                    "notebook",
                ...
                if is_mobile_query:
                    ...
                    is_prod_laptop = (prod.category_id == 1) and (
                        ...
                        or (
                            prod.name.split()[0].lower()
                            in ["dell", "hp", "lenovo", "asus", "acer", "apple"]
                        )
                    )
            ...
            if criteria["brand"]:
                ...
                if criteria["brand"] == "apple" or criteria["brand"] == "iphone":
                    if not (
                        "apple" in prod_name_lower
                        or "iphone" in prod_name_lower
                        or "apple" in prod_desc_lower
                    ):
                        continue
                else:
                    if (
                        criteria["brand"] not in prod_name_lower
                        and criteria["brand"] not in prod_desc_lower
                    ):
                        continue
            ...
            if price_matched:
                if budget_reasons:
                    reasons.append(
                        f"{' and '.join(budget_reasons)} (priced at ₹{prod.price:,.0f})"
                    )
                else:
            ...
            if criteria["ram"]:
                if (
                    criteria["ram"].lower() in prod_desc_lower
                    or criteria["ram"].lower() in prod_name_lower
                ):
                    score += 2
            ...
            if criteria["storage"]:
                if (
                    criteria["storage"].lower() in prod_desc_lower
                    or criteria["storage"].lower() in prod_name_lower
                ):
                    score += 2
            ...
            if criteria["display"]:
                if (
                    criteria["display"].lower() in prod_desc_lower
                    or criteria["display"].lower() in prod_name_lower
                ):
                    score += 2
                    reasons.append(
                        f"features a vivid {criteria['display'].upper()} display"
                    )
    ...
    async def generate_product_description(
        name: str, category: Optional[str] = None, price: Optional[float] = None
    ) -> str:
        """
    ...
    def _extract_criteria(message: str) -> Dict[str, Any]:
        ...
        if any(
            kw in message_clean
            for kw in ["mobile", "phone", "smartphone", "cellphone", "iphone"]
        ):
            criteria["category"] = "mobile"
        elif any(
            kw in message_clean for kw in ["laptop", "computer", "pc", "notebook"]
        ):
        ...
        min_match = re.search(
            r"(?:above|greater|more than|over|starting|min|at least)\s*(?:₹|rs\.?)?\s*(\d+)",
            message_clean,
        )
        max_match = re.search(
            r"(?:under|below|less|max|up to|budget|maximum)\s*(?:₹|rs\.?)?\s*(\d+)",
            message_clean,
        )
        ...
        storage_match = re.search(
            r"\b(64|128|256|512)\s*(?:gb)?\s*(?:storage|rom|ssd)\b", message_clean
        )
        if storage_match:
        ...
        if any(
            kw in message_clean
            for kw in ["camera", "mp", "megapixels", "photography", "photos"]
        ):
        ...
        if any(
            kw in message_clean
            for kw in ["battery", "mah", "long last", "charging", "charger"]
        ):
